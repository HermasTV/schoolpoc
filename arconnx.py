...
class faceRecognizer:
    ...
    def __init__(self,config_path) -> None:
        ...
        self.model_name = self.config["model_name"]["onnx"]
        ...
        if self.backend == "tritonserver":
            ...
            self.run = self.run_TRITONSERVER
    ...
    def pre_process(self, img: np.ndarray) -> np.ndarray:
        """Image pre-processing function
        Args:
            img (np.ndarray): a numpy image in BGR format
    ...
    def preprocess_(self, images: List[np.ndarray]): 
    ...
    def infer_TRITONSERVER_batch(self, processed_images, input_ports, output_ports):
        ...
        output0 = results.as_numpy(self.OUTPUT_NAMES[self.OUTPUT_NAMES.index("683")])

        return output0  

    def run_TRITONSERVER(self, images):
        processed_images = self.pre_process(images)
