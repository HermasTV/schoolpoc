...
class SCRFD:
    def __init__(self, configs, session=None):
        import onnxruntime
        self.configs = configs
        self.model_file = self.configs["model"]
    ...
    def predict(self, img, thresh=0.5, input_size = (640,640), max_num=0, metric='default'):
...
def detect(image):
    ...
    detector = SCRFD(model_file='/home/eslam_abdelrahman/school/SRCFD/model.onnx')
    detector.prepare(-1)
    ta = datetime.datetime.now()
    bboxes, kpss = detector.detect(image, 0.5, input_size = (640, 640))
    tb = datetime.datetime.now()
    time = round((tb-ta).total_seconds()*1000,3)
    print(f'time: {time},\tbboxes: {bboxes.shape},\tkpss: {kpss.shape}')
    return bboxes,kpss
